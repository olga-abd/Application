...
public class StaffDAO {
    ...
    public Staff findByLogin (String login) {
        Session session = HibernateUtils.getSesstionFactory().openSession();
        Staff employee = (Staff) session.createQuery("from pkg.staff.Staff where login = '" + login + "'").uniqueResult();
        session.close();
        return employee;
    }
    ...
    public List<Staff> getAllStaff () {
        Session session = HibernateUtils.getSesstionFactory().openSession();
        List<Staff> list = session.createQuery("from pkg.staff.Staff").list();
        session.close();
        return list;
    }
    ...
    public List<Employee> getSubordinatesById(int tn) {
        Session session = HibernateUtils.getSesstionFactory().openSession();
        List<Employee>  employees = session.createQuery("from pkg.staff.Employee where head_tabNum = " + tn).list();
        session.close();
        return employees;
    }
