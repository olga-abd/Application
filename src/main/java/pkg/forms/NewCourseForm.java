...
public class NewCourseForm extends JDialog{
    ...
    private JTextField tf_startDate3;
...
//        this.course = course;
        tf_startDate3.setInputVerifier(new DateInputVerifier());
    ...
    public class DateInputVerifier extends InputVerifier{
        ...
        public boolean verify(JComponent input) {
            ...
            try {
                ...
                if(tf_startDate3.getText().trim().length() > 0 && tf_endDate.getText().trim().length() > 0) {
                    if (dateFormat.parse(tf_startDate3.getText()).compareTo(dateFormat.parse(tf_endDate.getText())) > 0) {
    ...
    public class SaveButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            if (tf_center.getText().trim().length() > 0 && tf_center.isValid() &&
                ...
                tf_startDate3.getText().trim().length() > 0 && tf_startDate3.isValid()
            ) {
                try {
                    MainUtils.createNewCourse(tf_name.getText(),
                            tf_center.getText(),
                            new java.sql.Date(dateFormat.parse(tf_startDate3.getText()).getTime()),
